...
export function createPackageByItemPathPowershellTool(server: McpServer, config: Config) {
    server.tool(
        "packaging-create-package-by-item-path",
        "Creates a package from the item specified by path.",
        {
            name: z.string().describe("The name of the package to create."),
            itemPath: z.string().describe("The path of the item to create a package from."),
        ...
        async (params) => {
            ...
            getItemParameters["Path"] = params.itemPath;
